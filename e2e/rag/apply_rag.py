"""Apply Option A RAG FeatureView from driver Python (SparkSource needs JVM)."""
...
REPO_REF = os.environ.get("FEAST_EXAMPLES_REF", "feast-rag")
FEATURE_REPO = os.environ.get("FEAST_FEATURE_REPO", "feature_repo")
...
def main() -> None:
    ...
    work = Path(tempfile.mkdtemp(prefix="rag-apply-"))
    try:
        ...
        os.chdir(apply_root)
        print("=== feast apply (RAG Option A) ===", flush=True)
        subprocess.check_call(["feast", "apply"])
        print("PASS: city_embeddings registered", flush=True)
    finally:
